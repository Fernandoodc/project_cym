...
$('#guardaCambios').click(async function(){
    delete DATOS['_id']
    var descripcion = $('#descripcion')
    var precioBase = $('#precioBase')
    var metodoCalculo = $("#metodoCalculo :selected")
    ...
    $('#cancelar').removeClass('btn-secondary')
    $('#cancelar').addClass('btn-primary')
    $('#cancelar').html('Volver a Productos')
    $('#cancelar').click(function(){
...
function eliminarProducto(codProducto){
    ...
    $('#eliminarProducto-btn').click(async function(){
        await eliminarProductoAjax(codProducto)
        location.reload()
    })
}
